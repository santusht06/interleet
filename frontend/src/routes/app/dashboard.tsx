...
import { activityWeekly, challenges, recentActivity, user } from "@/lib/mock";
import { ChallengeCard } from "@/components/domain/ChallengeCard";

export const Route = createFileRoute("/app/dashboard")({
  head: () => ({ meta: [{ title: "Dashboard · Interleet" }] }),
  component: Dashboard,
});
    ...
    <AppShell>
      ...
      <div className="space-y-6 px-4 py-6 md:px-8">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <StatCard label="XP" value="48,210" delta="+1,240 this week" icon={Sparkles} accent="text-primary" />
          <StatCard label="Global rank" value="#327" delta="↑ 14 places" icon={Trophy} accent="text-chart-2" />
          <StatCard label="Streak" value={`${user.streak} days`} delta="Personal best" icon={Flame} accent="text-chart-3" />
          <StatCard label="Accuracy" value={`${user.accuracy}%`} delta="+3% vs last 30d" icon={Target} accent="text-chart-4" />
        </div>
          ...
          <Card className="border-border bg-card p-5 lg:col-span-2">
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Challenges solved and minutes practiced.</p>
              </div>
              <Badge variant="outline" className="font-mono text-[10px]">7d</Badge>
            </div>
            <div className="h-64">
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <div className="lg:col-span-2">
            <div className="mb-3 flex items-center justify-between">
              ...
              <Link to="/app/challenges" className="text-xs text-muted-foreground hover:text-foreground">
                See all <ChevronRight className="ml-0.5 inline h-3 w-3" />
              </Link>
            </div>
          ...
          <Card className="border-border bg-card p-5">
            ...
            <Button variant="ghost" className="mt-3 w-full">
              View all activity <ArrowUpRight className="ml-1 h-3.5 w-3.5" />
            </Button>
          </Card>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <Card className="border-border bg-card p-5 lg:col-span-2">
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Rolling average across all sessions.</p>
              </div>
              <Badge variant="outline" className="font-mono text-[10px]">90d</Badge>
            </div>
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart
          ...
          <Card className="border-border bg-card p-5">
            ...
            <div className="space-y-2">
              {user.badges.map((b) => (
                <div key={b} className="flex items-center gap-3 rounded-md border border-border bg-background/40 p-2.5">
