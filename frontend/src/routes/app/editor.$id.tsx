...
export const Route = createFileRoute("/app/editor/$id")({
  head: ({ params }) => ({ meta: [{ title: `Editor · ${params.id}` }] }),
  loader: ({ params }) => {
      ...
      <div className="flex items-center justify-between border-b border-border bg-card/40 px-4 py-2 md:px-6">
        <div className="flex min-w-0 items-center gap-3">
          <Button asChild variant="ghost" size="icon" className="h-8 w-8">
            <Link to="/app/challenges/$id" params={{ id: c.slug }}>
...
function BrowserPreview({ domain, slug, title }: { domain: string; slug: string; title: string }) {
  return (
...
function PreviewArea({ domain, slug, title }: { domain: string; slug: string; title: string }) {
...
function getProgramOutput(slug: string): {
  log: string;
...
function getFrontendSrcDoc(slug: string, title: string): string {
  const base = `
    <style>
