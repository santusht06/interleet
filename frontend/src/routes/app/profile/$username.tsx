...
export const Route = createFileRoute("/app/profile/$username")({
  head: ({ params }) => ({ meta: [{ title: `@${params.username} · Interleet` }] }),
      ...
      <div className="relative border-b border-border">
        ...
        <div className="relative mx-auto flex max-w-6xl flex-col gap-5 px-4 py-8 md:flex-row md:items-end md:px-8">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">@{username} · {user.title}</p>
            <div className="mt-2 flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
              <span className="inline-flex items-center gap-1"><MapPin className="h-3.5 w-3.5" />{user.location}</span>
              <span className="inline-flex items-center gap-1"><Github className="h-3.5 w-3.5" />alex-morgan</span>
              <span className="inline-flex items-center gap-1"><LinkIcon className="h-3.5 w-3.5" />alexmorgan.dev</span>
            </div>
          </div>
        ...
        <Tabs defaultValue="overview" className="mt-6">
          ...
          <TabsContent value="overview" className="mt-4 grid gap-4 lg:grid-cols-3">
            ...
            <Card className="border-border bg-card p-5">
              ...
              <div className="mt-4 grid grid-cols-26 gap-1" style={{ gridTemplateColumns: "repeat(26, minmax(0, 1fr))" }}>
                {Array.from({ length: 26 * 7 }).map((_, i) => {
                  const v = (Math.sin(i * 1.3) + 1) / 2;
                  const bucket = Math.floor(v * 4);
                  const cls = ["bg-muted", "bg-primary/25", "bg-primary/55", "bg-primary/80", "bg-primary"][bucket];
                  return <span key={i} className={`h-2.5 w-2.5 rounded-sm ${cls}`} />;
                })}
              </div>
              <p className="mt-3 text-xs text-muted-foreground">Last 6 months · 412 contributions</p>
            </Card>
          </TabsContent>
                ...
                {challenges.slice(0, 6).map((c) => (
                  <li key={c.id} className="flex items-center justify-between px-4 py-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{c.domain} · {c.difficulty}</p>
                    </div>
                    <Badge variant="outline" className="font-mono">{c.xp} XP</Badge>
                  </li>
                ))}
              </ul>
            </Card>
                ...
                {interviewHistory.map((iv) => (
                  <li key={iv.id} className="flex items-center justify-between px-4 py-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{iv.when} · {iv.duration}m</p>
                    </div>
          ...
          <TabsContent value="badges" className="mt-4 grid gap-3 sm:grid-cols-2 md:grid-cols-4">
            {user.badges.concat(["Frontend I", "Database Pro", "Distributed II", "Contest Finalist"]).map((b) => (
              <Card key={b} className="flex items-center gap-3 border-border bg-card p-4">
                <span className="flex h-9 w-9 items-center justify-center rounded-md bg-primary/15 text-primary"><Award className="h-4 w-4" /></span>
